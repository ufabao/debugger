...
std::int64_t get_section_load_bias(std::filesystem::path path, Elf64_Addr file_address)
{
  ...
  pclose(pipe);
  mdb::error::send("Could not find section load bias");
  return 0;
}
...
virt_addr get_load_address(pid_t pid, std::int64_t offset)
{
  ...
  mdb::error::send("Could not find load address");
  // Since we can't reach here (error::send likely throws), return a default value to satisfy the
  // compiler
  return virt_addr(0);
}
...
TEST_CASE("Can iterate breakpoint sites", "[breakpoint]")
{
  ...
  cproc->breakpoint_sites().for_each([addr = 42](const auto& site) mutable
                                     { REQUIRE(site.address().addr() == addr++); });
}
...
TEST_CASE("Breakpoint on address works", "[breakpoint]")
{
  ...
  auto proc = process::launch("targets/hello_mdb", true, channel.get_write());
  channel.close_write();

  auto offset       = get_entry_point_offset("targets/hello_mdb");
  ...
  REQUIRE(to_string_view(data) == "Hello, mdb!\n");
}
...
TEST_CASE("Reading and writing memory works", "[memory]")
{
  ...
  proc->write_memory(virt_addr{b_pointer}, {as_bytes("Hello, mdb!"), 12});
  proc->resume();
  proc->wait_on_signal();

  auto read = channel.read();
  REQUIRE(to_string_view(read) == "Hello, mdb!");
}
...
TEST_CASE("Watchpoint detects read", "[watchpoint]")
{
  ...
  auto& watch = proc->create_watchpoint(func, mdb::stoppoint_mode::read_write, 1);
  watch.enable();
...
{
  REQUIRE(mdb::syscall_id_to_name(0) == "read");
  REQUIRE(mdb::syscall_name_to_id("read") == 0);
  REQUIRE(mdb::syscall_id_to_name(1) == "write");
  REQUIRE(mdb::syscall_name_to_id("write") == 1);
  REQUIRE(mdb::syscall_id_to_name(62) == "kill");
  REQUIRE(mdb::syscall_name_to_id("kill") == 62);
}
...
TEST_CASE("Syscall catchpoints work", "[catchpoint]")
{
  ...
  auto write_syscall = mdb::syscall_name_to_id("write");
  auto policy        = mdb::syscall_catch_policy::catch_some({write_syscall});
  proc->set_syscall_catch_policy(policy);

  proc->resume();
  auto reason = proc->wait_on_signal();

  REQUIRE(reason.reason == mdb::process_state::stopped);
  REQUIRE(reason.info == SIGTRAP);
  REQUIRE(reason.trap_reason == mdb::trap_type::syscall);
